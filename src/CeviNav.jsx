...
export default function CeviNav({stufeAct}) {

  const [stufenIsActive, setStufenIsActive] = useState(false);  

  const activeClassName = 'bg-gradient-to-r from-cevi-red to-cevi-whiteRed h-10 m-5 rounded-xl flex items-center drop-shadow-lg hover:text-cevi-grayWhite text-cevi-white';
  const nonActiveClassName = 'bg-cevi-white h-10 m-5 rounded-xl flex items-center hover:bg-cevi-grayWhite text-cevi-gray';


  function BurgerOpenStufe(){
    console.log("openStufe")
    setStufenIsActive(true)
  }

  function BurgerCloseStufe(){
    console.log("closeStufe")
    setStufenIsActive(false)
  }
    ...
    <>
      <div className='flex'>
        <nav className='w-64 top-0 bg-cevi-white h-screen sticky drop-shadow-2xl'>
            ...
            <NavLink onMouseOver={BurgerOpenStufe} onMouseOut={BurgerCloseStufe} to="stufen" className={({ isActive }) =>isActive ? activeClassName : nonActiveClassName}>
              {({ isActive }) => (
                <>
            ...
            <NavLink  to="aboutUs" className={({ isActive }) =>isActive ? activeClassName : nonActiveClassName}>
              {({ isActive }) => (
                <>
